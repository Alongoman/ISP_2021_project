function [valid_mask,BB,hue_low_th,hue_high_th,sat_low_th,sat_high_th]= initial_bracelet_rep_hs(img,point,color)
    h=250;
    w=250;
    err=0.5;
    ...
    hue_low_th = miu_h-sqrt(2*sigma_h^2*(err-0.5*log(2*pi*sigma_h^2)));
    hue_high_th = miu_h+sqrt(2*sigma_h^2*(err-0.5*log(2*pi*sigma_h^2)));
...
%%
    ...
    mask_tot =logical((sat_tot>=sat_low_th).*(sat_tot<sat_high_th).*(hue_tot>=hue_low_th).*(hue_tot<hue_high_th));
